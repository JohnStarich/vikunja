...
func TestTaskCollection(t *testing.T) {
	...
	t.Run("ReadAll on project", func(t *testing.T) {
		...
		t.Run("Search prefix match", func(t *testing.T) {
			// Issue #2346: searching "landing" should match "Landingpages update"
			rec, err := testHandler.testReadAllWithUser(url.Values{"s": []string{"landing"}}, urlParams)
			require.NoError(t, err)
			assert.Contains(t, rec.Body.String(), `Landingpages update`)
			assert.NotContains(t, rec.Body.String(), `task #1`)
		})
		t.Run("Search prefix match case insensitive", func(t *testing.T) {
			rec, err := testHandler.testReadAllWithUser(url.Values{"s": []string{"LANDING"}}, urlParams)
			require.NoError(t, err)
			assert.Contains(t, rec.Body.String(), `Landingpages update`)
		})
		t.Run("Search prefix match in description", func(t *testing.T) {
			rec, err := testHandler.testReadAllWithUser(url.Values{"s": []string{"branding"}}, urlParams)
			require.NoError(t, err)
			assert.Contains(t, rec.Body.String(), `Landingpages update`)
		})
		t.Run("Sort Order", func(t *testing.T) {
			// TODO: Add more cases
			// should equal urgency
			t.Run("by urgency", func(t *testing.T) {
				rec, err := testHandler.testReadAllWithUser(url.Values{"sort_by": []string{"urgency"}, "order_by": []string{"desc"}}, urlParams)
				require.NoError(t, err)
				var tasks []models.Task
				require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &tasks))

				var ids []int64
				var urgencyScores []float64
				for _, task := range tasks {
					ids = append(ids, task.ID)
					urgencyScores = append(urgencyScores, *task.Urgency)
				}
	...
	t.Run("ReadAll for all tasks", func(t *testing.T) {
		...
		t.Run("Search prefix match", func(t *testing.T) {
			rec, err := testHandler.testReadAllWithUser(url.Values{"s": []string{"landing"}}, nil)
			require.NoError(t, err)
			assert.Contains(t, rec.Body.String(), `Landingpages update`)
		})
		t.Run("Sort Order", func(t *testing.T) {
